...
PosSistem_kemudi.createSistem_kemudi = (data, result) =>{
    ...
    async function main() {
        ...
        try {
            ...
            const issueResponse = await contract.submitTransaction('create', data.no_pemeriksaan, data.no_kendaraan, data.roda_kemudi.toString(), data.speling_roda_kemudi.toString(), data.batang_kemudi.toString(), data.roda_gigi_kemudi.toString(), data.sambungan_kemudi.toString(), data.penyambung_sendi_peluru.toString(), data.power_steering.toString(), data.slide_slip.toString(), data.notes, data.status);
