...
PosSuspension.createSuspension = (data, result) =>{
    ...
    async function main() {
        ...
        try {
            ...
            const issueResponse = await contract.submitTransaction('create', data.no_pemeriksaan, data.no_kendaraan, data.suspensi_roda_depan.toString(), data.suspensi_roda_belakang.toString(), data.sumbu.toString(), data.pemasangan_sumbu.toString(), data.pegas.toString(), data.bantalan_roda.toString(), data.notes, data.status);
