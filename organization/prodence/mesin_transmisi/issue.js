...
PosMesin_transmisi.createMesin_transmisi = (data, result) =>{
    ...
    async function main() {
        ...
        try {
            ...
            const issueResponse = await contract.submitTransaction('create', data.no_pemeriksaan.toString(), data.no_kendaraan.toString(), data.dudukan_mesin.toString(), data.kondisi_mesin.toString(), data.transmisi.toString(), data.sistem_gas_buang.toString(), data.emisi_asap.toString(), data.emisi_co.toString(), data.status);
